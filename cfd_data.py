...
INSTRUMENTS = [
    "USD_CHF", "NAS100_USD", "EUR_USD", "XAU_USD", 
    "GBP_USD", "USD_JPY", "AUD_USD", "USD_CAD",
    "EUR_GBP", "EUR_JPY", "GBP_JPY",
    "XAG_USD",  "AUD_CAD", # Gold and Silver
    "US30_USD", "SPX500_USD", "UK100_GBP"  # More indices
]
GRANULARITY = "M1"
...
# Process each instrument one by one
for instrument in INSTRUMENTS:
    ...
    if start_datetime >= end_datetime:
        print(f"Data for {instrument} is already up to date!")
    ...
    # Save the new data for this instrument
    if not new_data_df.empty:
        print(f"\nDownload complete for {instrument}. Found {len(new_data_df)} new records.")
        ...
        print(f"\nNo new data was downloaded for {instrument}. Your data is already up to date!")
    
    print(f"Completed processing {instrument}")
    
    # Add a small delay between instruments to be gentle on the API
    if instrument != INSTRUMENTS[-1]:  # Don't delay after the last instrument
        print("Waiting 5 seconds before processing next instrument...")
        time.sleep(5)

print(f"\n{'='*60}")
print("All instruments have been processed!")
print(f"{'='*60}")